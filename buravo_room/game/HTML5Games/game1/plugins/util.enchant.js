// forked from usamyu_'s "utit.enchant.js(ver.0.2)" http://jsdo.it/usamyu_/util
/**
...
enchant.util = { assets: ['data/effect0.gif', 'data/icon0.gif', 'data/font.png'] };
// 背景専用スプライト
...
enchant.util.MutableText = enchant.Class.create(enchant.Sprite, {
	...
	setText: function(txt) {
		...
		for(i=0; i<txt.length; i++) {
			...
			this.image.draw(enchant.Game.instance.assets['data/font.png'], 
...
enchant.util.LifeLabel = enchant.Class.create(enchant.Group, {
	initialize: function(x, y, maxlife) {
		...
		for(var i=0; i<this._maxlife; i++){
			...
			this.heart[i].image = enchant.Game.instance.assets['data/icon0.gif'];
...
enchant.Bar = enchant.Class.create(enchant.Sprite, {
	initialize: function(x, y) {
		...
		// this.image.context.fillStyle = 'rgb(255, 0, 0)';
